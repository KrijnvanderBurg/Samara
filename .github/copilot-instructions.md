# Samara
Samara is a Python framework for building ETL data pipelines through JSON/YAML configuration files instead of code. Users define data sources, transformations, and outputs in config files. Samara parses and validates these configs using Pydantic models, then executes the pipeline on Apache Spark (with Polars support in development).
